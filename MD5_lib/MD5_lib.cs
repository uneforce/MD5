...
namespace MD5_lib
{
    public interface MD5_Hash
    {
        void GetHash(Stream input, Stream output);
    }

    public class MD5 : MD5_Hash
    {
        ...
        protected static byte[] byteInput;

        public void GetHash(Stream input, Stream output)
        {
            byteInput = new byte[input.Length];
            input.Position = 0;
            int size = input.Read(byteInput, 0, (int)input.Length);

            string hash = MD5Value().ToString();

            output.Write(Encoding.Default.GetBytes(hash), 0, Encoding.Default.GetBytes(hash).Length);
        }

        protected Digest MD5Value()
        {
            byte[] bMsg;
            uint n;
        ...
        public class Digest
        {
            ...
            public Digest()
            {
                A = (uint)MD5_Constant.A;
                B = (uint)MD5_Constant.B;
                C = (uint)MD5_Constant.C;
                D = (uint)MD5_Constant.D;
            }
